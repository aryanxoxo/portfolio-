...
int main(void)
{
  ...
  fft(1, EXPONENT, readings, complex_component);
                                            ...
                                            */
  for (i = 0; i < NUMBER_OF_READINGS; i++) {
	  readings[i] = pow((pow(readings[i], 2) + pow(complex_component[i], 2)), (1 / 2));
...
fopen_s(&file_pointer, "result.txt", "w");

                                            /* Writes the result to the file */
fprintf( file_pointer, "Puddle jump Tidal Frequency : %f per day", frequency);
...
void process_file(double array_to_populate[], FILE* pointer_to_data_file)
{
  ...
  while (fgets(line_buffer, LINESIZE, pointer_to_data_file)) {
	 ...
	 while (m = 0, m < MAX_VALUES_PER_LINE, m++) {
		  values_per_line = sscanf_s(line_buffer, "%d", &extracted_values[m]);
	  }
	  ...
	  for (i = 0; i < values_per_line; i++) {
		  array_to_populate[n] = extracted_values[i];
		  n++;
	  }
