/*
 ...
 Author:		Aryan Chahardovalee & Sidharth Chadhaqq
...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates)
{
  ...
  for (i = 0; i < number_of_candidates; i++) {
	  char* P_candidate = *(candidate_segments+i);
	  if (strcmp(sample_segment, P_candidate) == 0) {
		  has_perfect_match = i + 1;  // identifies the perfect match and returns it's candidate number immidietley 
	 ...
	 // Insert your code here
  if (count != 0) { //if no perfect matches were found then we calculate the score ...
	  return;
  } else {
	  for (i = 0; i < number_of_candidates; ++i) { // using two different: methods one with a big for loop and one with two smaller for loops 
		  char* O_candidate = *(candidate_segments+i);
		  a_score = calculate_score_a(sample_segment,O_candidate); // compares score using the first method
		  printf("Candidate number %d matched with a best score of %d using the fisrt method.\n", i + 1, a_score);
...
int calculate_score_a(char* sample_segment, char* candidate_segment)
{
	...
	int i, j, m;
	int number_of_extra_nucleotides;
	int sscore = 0;

	if (sample_length % 3 != 0) {  //if statement checks to see whether the total number of nucleotides present in a sample is a factor of 3 or not.
		number_of_extra_nucleotides = sample_length % 3; //this variable saves the number of extra nucleotides.
		for (i = 0; i < number_of_extra_nucleotides; i++) {
			*(sample_segment + sample_length - 1 - i) = '/0';
		} // This omits any excess nucleotides in the string by replacing them with the null character.
		sample_length = strlen(sample_segment);
	}
	int S_LENGTH = sample_length / 3;      //calculates the number of iteration or the number of times we should move the candidate segment
	int C_LENGTH = candidate_length / 3;
	iterations = C_LENGTH - S_LENGTH + 1;


	for (j = 0; j < iterations; j++) {  // each iteration 
		
		for (i = 0; i < S_LENGTH; i++) {// each length 
			
			if ((strncmp(sample_segment + (3 * i) + j, candidate_segment + (3 * (j + i)), 3)) == 0) { //compares the similarity of codons and returns 10 points 
				temp_score += 10;
			}
			else if (get_codon_index(sample_segment + (3 * i) + j) == get_codon_index(candidate_segment + (3 * (j + i)))) { //compares type of each codon and returns 5 points 
				temp_score += 5;
			}
			else if (temp_score==0){
				for (m = 0; m < sample_length; m++) {
					if ((sample_segment[m]) == (candidate_segment[m + (3 * (j+i ))])) { //goes over every single nucleotides and checks for similarity
						temp_score += 2;
					}
					else if (is_base_pair(sample_segment[m], candidate_segment[m + (3 * (j +i))]) == 1) { //goes over every nucleotides and searches for compatiblity
						temp_score += 1;                                                                            // A to T and C to G ... using the function is_base_pair
					}
		...
		temp_score = 0;// re adjusts the value for the next iteration 
	}
